...
@Named
@ViewScoped
public class SolicitudManualAdministrativoController implements Serializable, IControllerOld {
    ...
    public void move(Integer page) {
        ...
        try {
            ...
            switch (loginController.get("searchsolicitud")) {
                case "_init":
                    doc = new Document("tiposolicitud.idtiposolicitud", "ADMINISTRATIVO");
    ...
    private Boolean foundTipovehiculo(String idtipovehiculo) {
        ...
        try {
            Tipovehiculo tipovehiculo = tipovehiculoList.stream() // Convert to steam
                    .filter(x -> x.getIdtipovehiculo().equals(idtipovehiculo)) // we want "jack" only
                    .findAny() // If 'findAny' then return found
                    .orElse(null);
